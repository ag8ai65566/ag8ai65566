...
**圖生影片**（丟圖 + 指令 → 幾秒動畫）和**文生圖片**（打提詞 → 圖）兩條線，共用一套模型、LoRA 和素材管理。**完全在你自己的機器上跑** —— 裝好之後
...
可調的東西都有一句話解釋在旁邊：步數、CFG、sampler、scheduler、CLIP skip、
尺寸、一次幾張（批量）、種子、高解析重繪。不想調就不用碰，預設值是照每個底模的
建議填的。
...
app/
  ...
  images.py            圖片底模目錄 + SDXL 文生圖工作流程 + 每個旋鈕的一句話解釋
