...
| `wan22-s2v` | Wan 2.2 S2V 14B | 16GB | 24GB | **說話鏡頭**：音訊驅動、原生口型，**只下載檔案** |
| `wan22-animate` | Wan 2.2 Animate 14B | 16GB | 27GB | **動作轉移**：把參考影片的動作套到你的角色，**只下載檔案** |
| `minimax-h3` | MiniMax H3 FL2VA | 24GB | **56GB** | 首尾幀＋原生同步音訊，**授權排除美國／歐盟／英國／韓國**，**只下載檔案** |
...
### MiniMax H3：權重是真的開放的，但有兩個陷阱

常有人問「MiniMax H3 效果好像很接近 Seedance,可以用嗎」。**權重確實是開放的**
（`Comfy-Org/MiniMax-H3`,ComfyUI 官方打包）,這點跟 Seedance 完全不同。但:

1. **授權排除了美國、歐盟、英國、韓國。** 這是這份目錄裡唯一排除美國的授權,
   而且是自訂社群授權,商用還有額外條款。下載按鈕前會要你先確認讀過。
2. **它是目錄裡最大的（約 56GB）**,光文字編碼器（Qwen3-VL 32B）就 27GB,
   塞不進 24GB 顯存,一定要大量 offload。而且**它沒有單純的 I2V** ——
   FL2VA 是首尾幀。

成人 LoRA 生態也還比 Wan 2.2 小（查證當日 67 個 vs Wan 的 100+,
而且 Wan 那邊才有互動類的）。所以它是加進來了,但**不是短劇的預設路線**。
...
難的是一集有上百顆鏡頭，而它們要看起來是同一個人在同一個房間裡，那是**記帳**不是提詞。

一個專案 = 一集。分頁按流程排：

- **第 0 步 鎖規格** —— 選一組「影片模型＋原生幀率」的**組合**，不能只選模型。
  混幀率正是那些 AI 短劇平台最明顯的破綻，而那是他們**被內容審核逼出來的**
  （一半鏡頭得走商業 API，另一半只能本機跑，兩軌撞在同一條時間線上）。
  你整條線都在本機，沒有理由繼承這個妥協。
- **第 1 步 資產層** —— 卡司表：觸發詞、**鎖定服裝**、專屬 LoRA。
  角色一致性：閉源參考圖 85-90%、IP-Adapter＋ControlNet 90-95%、**專屬 LoRA 接近 100%**，
  而**人工精修佔總工時 40% 以上**，那 40% 幾乎全是一致性沒鎖好的帳。
- **第 2 步 分鏡表** —— 一列一顆鏡頭，即時算總長、**鏡頭中位數**、台詞字數。
  產業慣例：直式 9:16、單集 1-3 分、**鏡頭中位數 3.2 秒**（8 分鐘 = 117 顆）、
  單句台詞 ≤15 字、開場**黃金 3 秒**。
- **分鏡健康度** —— 跟提詞健康度同一套：幀率對不上、鏡頭太長、有台詞卻沒用 S2V、
  角色沒 LoRA、模型沒下載、關鍵幀太小或是橫的⋯⋯每條都是可查證的規則。
- **做這顆** —— 把那顆鏡頭的關鍵幀提詞照 danbooru caption 的順序組好
  （角色觸發詞→鎖定服裝→景別 tag→場景→動作，並去掉重複的 tag），
  列出步驟和理由，一鍵送到圖片分頁。

**它不幫你生成** —— 生成還是在圖片／影片分頁，那裡才有完整的工具。
它也**不會說「這樣會好看」**：這個專案沒有生成過任何影片，沒有資格講那種話。
