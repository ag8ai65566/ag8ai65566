...
| `minimax-h3` | MiniMax H3 | 24GB | **44GB** | 圖生影片＋首尾幀，**自帶同步聲音**（模型生的，不是你放的），**授權排除美國／歐盟／英國／韓國**。工作流照 ComfyUI 內建範本 `video_minimax_h3_i2v` 一比一組的，**本專案沒有 GPU，沒有實跑驗證過** |
...
HunyuanVideo 1.5 放進來是因為它的 DiT 只有 8.3B 參數（主模型檔 8.3GB，全套 21.5GB），
顯存門檻是目錄裡最低的一批 —— 官方寫 bf16 開 offload 最低 14GB，這裡下載的是 fp8 量化版，
理論上更低，但這台機器沒有 GPU，沒實測過。人臉和物理也比較自然。NSFW 生態比 Wan 少，但當備選很有用。
...
2. **它是目錄裡最大的（約 44GB）**,光文字編碼器（Qwen3-VL 32B,nvfp4）就 15.7GB,
   塞不進 24GB 顯存,一定要大量 offload。

它**可以在這裡直接跑**（圖生影片,也支援首尾幀）,工作流是照 ComfyUI 內建範本
`video_minimax_h3_i2v` 一比一組的 —— 但跟這裡每一張圖一樣,只驗到結構,
**本專案沒有 GPU,一格都沒實跑過**。聲音是它自己生的,不是你放進去的。
...
分頁最上面是**「做一段影片」**：上傳圖 → 選模型 → 打指令 → 生成，就這樣。
不用建專案、不用分鏡表。要排一整集才往下拉 —— 下面那一段是收合起來的。
...
- **整集** —— 一次只亮一顆按鈕，上面那行字直接說現在該做哪一步：
  ① 排完所有關鍵幀 →（你自己挑）→ ② 排完所有影片 →（再挑一次）→ ③ 接成一支影片。
  一次只送幾個工作進佇列（這台機器只有一個 worker，塞滿不會比較快，
  只會讓你想單獨試一顆時排在整集後面）；某一顆失敗會繼續跑其他顆，原因列在下面。
  **它不會替你採用任何一個候選** —— 最後跑完的那個不是最好的那個。
- **接成一支影片** —— 每顆先轉成交付規格，再無損接起來。
  `ffmpeg -f concat` 要求所有輸入的編碼、尺寸、幀率已經一致，而你的鏡頭不會一致：
  對白走 S2V、動作走 I2V，是兩個不同的 checkpoint。16fps 和 24fps 直接接，
  後半段會用錯的速度播。沒有音軌的鏡頭補等長靜音，不然接起來會整支掉音軌。
  **需要你自己有 ffmpeg**（找不到會告訴你去哪裡下載）。

**單顆鏡頭的生成不幫你決定任何事** —— 每一步都要你按下去，
而且**沒有東西會自己被採用**。它也**不會說「這樣會好看」**：
這個專案沒有生成過任何影片，沒有資格講那種話。
...
app/
  ...
  runner.py            整集批次：排完一個階段就停下來等你挑，一次只送幾個工作進佇列
  assemble.py          把採用的鏡頭轉成交付規格後接成一支檔案（需要你自己有 ffmpeg）
