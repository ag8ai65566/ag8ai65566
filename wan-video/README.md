...
4. 把量到的格子當成一個一次性版型，每格提詞都填好給你，**還可以再改**

它也會**明講自己在猜哪裡**：吻合度百分比、哪些原角色特徵被換掉了，都列出來。
遇到斜的格子、破格、滿版單張，它不會硬掰 —— 格溝寬度差太多的時候會直接說
「這比較像一張滿版圖」，然後讓你手動挑一個接近的版型。這是量出來的判斷：
本頁所有內建版型的格溝寬度差異是 0.00～0.20，一張被誤切成六格的滿版圖是 9.97。
