...
要不要加前綴是**按 MBL 家族**決定的,而來源報表看不出任何線索 —— 只能從 RA
實際接不接受來learn。8/25 那次上傳確認到:

| 船公司 + MBL 開頭 | RA 要的格式 | 怎麼知道的 |
| --- | --- | --- |
| `PABV` + `NGB…` | **要前綴** `PABVNGB…` | 上傳成功,沒有錯誤 |
| `PABV` + `SHCS…` | **不要前綴**,維持 `SHCS…` | 加了前綴的 9 筆全被 RA 退:`Container No ... is not exists` |
| `CMDU` + `CHN…` | 推測要前綴 | **尚未驗證** — 來源檔本身有 5 筆已經寫成 `CMDUCHN…`,8 筆寫成 `CHN…` |

`SHCS` 這條已經寫進 `core.js` 的 `NO_PREFIX`,工具不會再對它加前綴。

以後如果 RA 又退某個家族,在 `NO_PREFIX` 加一行就好:
