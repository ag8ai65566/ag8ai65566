...
export function deriveMetrics(f) {
  ...
  const eq = last('equity'), assets = last('assets'), liab = last('liabilities');
  ...
  return {
    ...
    plausibility: (() => {
      ...
      // RECONCILIATION — the balance sheet does not close, but not by enough to prove a
      // fetch error. Mezzanine items (redeemable non-controlling interests) legitimately
      // sit between liabilities and equity, so a strict equality would false-positive on
      // ordinary filings. A gap beyond tolerance means a human should read the statement.
      // NOTE: this tier was unreachable in its first version — `liabilities` was never
      // fetched, so the middle grade of a three-grade system could never fire.
      const BALANCE_TOLERANCE = 0.05;
      if (assets && eq && liab && assets.end === eq.end && assets.end === liab.end) {
        const gap = assets.val - (liab.val + eq.val);
        const rel = Math.abs(gap) / assets.val;
        if (rel > BALANCE_TOLERANCE) {
          reconciliation.push({ check: 'balance_sheet_closes',
            gap_usd: gap, gap_pct_of_assets: Number((rel * 100).toFixed(2)),
            detail: `資產 ${(assets.val / 1e9).toFixed(2)}B ≠ 負債 ${(liab.val / 1e9).toFixed(2)}B + 權益 ${(eq.val / 1e9).toFixed(2)}B，`
              + `差額 ${(gap / 1e9).toFixed(2)}B（占資產 ${(rel * 100).toFixed(2)}%）`,
            issue: '資產負債表未在容忍範圍內結平。夾層項目（可贖回非控制權益）會造成小額差異，'
              + '超過容忍值則可能是抓錯實體、期間或 tag。',
            action: 'VERIFY_AGAINST_FILING', affects: ['roe_annualised_pct', 'assets_usd'] });
        }
      }
      ...
      return {
        ...
        needs_human_review: reconciliation.flatMap((x) => x.affects ?? []),
