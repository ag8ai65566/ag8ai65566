...
export async function aggregateCapex(opts = {}) {
  ...
  return {
    issuers: issuers.map((i) => ({
      ticker: i.ticker, tag: i.tag, raw_hash: i.raw_hash, retrieved_at: i.retrieved_at,
      quarters_available: i.quarters.length,
      // The freshness check needs to know how far each issuer has actually filed, not just
      // how many quarters exist — an aggregate that stops early must be able to say
      // whether that is the source's lag or this pipeline's.
      quarters: i.quarters.map((q) => ({ end: q.end, filed: q.filed, derived: q.derived })),
      latest_period_end: i.quarters.at(-1)?.end ?? null,
