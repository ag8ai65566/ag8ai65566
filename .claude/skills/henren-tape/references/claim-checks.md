...
## 2026-08-26 · 這份 log 自己出的問題：門檻不是他畫的

這一段檢的不是他的說法，是**本系統宣稱在用他的門檻**這件事。

起因：SOFR−IORB 從 0 走到 +1bp，儀表板把它從綠燈翻成黃燈。查證時發現**他的原話就印在
同一張卡片上**：

> 「SOFR-IRB 這個大家一定要每天都看，**黃色預警是利差衝過三個基點**，因為這是一個 95 分位的程度。」

而系統的門檻是綠 <1bp、黃 1–3bp、紅 ≥3bp ——
**把他的黃燈畫成紅燈，又自己加了一條他沒說過的 1bp 黃線。**
那個「翻黃燈」翻的是自訂線。改回他的原話後：**綠燈**，而且近 20 個交易日**一次都沒到 3bp**。

順著加了一個測試（每條數值門檻要嘛在引文裡，要嘛標明是本系統畫的），一跑抓出六格：

| 門檻來源 | 格數 | 是哪幾格 |
| --- | --- | --- |
| 他給的數值或規則 | 6 | 準備金 2.8/2.5 兆、capex 環比方向、SOFR 3bp、TGA 9500 億、內部人比 5 倍、預估本益比 30 倍 |
| **由他的判定反推** | 3 | M2 增負差、0DTE、前十大權重 |
| **整條線都是本系統的** | 6 | 開關二 0.95、RRP、HY OAS、NFCI、AI 籃子、巴菲特 150/200 |

**16 格裡只有 6 格用的是他真正說過的數字。**

### 最嚴重的一格：規則和他自己的例子矛盾

> 「第五項 M2 同比增長 5.6% 而同期的納斯達克漲幅是 29.7%，**增負差仍在負的 24.1%，這項也觸發了**。」

系統的紅線設在 ≤ −25 —— **他親口判定「觸發」的 −24.1，在本系統裡會被算成黃燈。**
規則與它唯一的已知案例對不上。紅線已改為 ≤ −24（由他的判定反推）。

### 這件事對這份 log 的意義

這份 log 上半部的結論是「他的可檢查說法幾乎每一條都落地」，那個結論不變 ——
**出問題的是本系統，不是他。** 但它改變了另一件事：

之前寫的工作守則是「**借用結構，每個數字自己重新推導**」。
現在要多一條：**借用門檻時，要能指出他在哪句話裡講過那個數字；
指不出來，就必須標成自己的。** 反推的門檻（讓他的判定成立）是第三種，
它比自訂強、比原話弱，不能混在一起講。
